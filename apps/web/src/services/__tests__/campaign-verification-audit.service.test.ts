...
} from '../campaign-verification-audit.service';

describe('CampaignVerificationAuditService', () => {
  let tmpDir: string;
  let auditService: CampaignVerificationAuditService;
  let mockPushService: VerificationNotifier;
  let pushNotificationsDispatched: Array<{
    campaignId: string;
  ...
  beforeEach(async () => {
    ...
    mockPushService = {
      sendMilestonePushNotification: async (params) => {
        pushNotificationsDispatched.push(params);
        return { sentCount: 1 };
      },
    };
  ...
  it('logs approval, updates status to approved, and triggers verification_complete push notification', async () => {
    ...
    expect(pushNotificationsDispatched[0].milestone).toBe('verification_complete');
    expect(pushNotificationsDispatched[0].campaignTitle).toContain('Kilimanjaro Reforestation');
  });
